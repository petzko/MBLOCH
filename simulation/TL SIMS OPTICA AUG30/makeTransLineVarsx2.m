function [ dat ] = makeTransLineVarsx2(settings,dat)
...
dat.eps_ch = 1E15*Constants('eps0'); mu_ch  = 1E3*Constants('mu0');
...
%   TL impedence
dat.Z0 = sqrt(mu_ch/dat.eps_ch)/settings.nRF;
...
dat.v_TL_old = dat.v_TL; dat.v_tmp = zeros(settings.N+1,1,dat.dtype);
...
dat.Acoeff = (settings.nTHz/settings.nRF)^2; dat.Bcoeff = dat.dt^2/dat.eps_ch/(settings.nRF^2);
dat.Dcoeff = (settings.nTHz/settings.nRF)/dat.Z0;
dat.Ecoeff = (settings.nTHz/settings.nRF)*dat.Z0;
dat.Fcoeff = dat.dt/dat.eps_ch/(settings.nRF^2);
