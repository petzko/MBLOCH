function [ savename ] = runsim_RING_RNFD_multilevelHTB_dispcomp3AR( scenarioFile,simDataFile,varargin )
...
info.N = N;
info.l_0 = l_0; 
info.SIMTYPE = 'WITH 2lvl DISPERSION COMPENSATION';
...
while(t< tEnd)
    ...
    U = wave_solver.make_step(-1i*c*(n32+(nA+nB)),-1i*c*(n32_t+(nA_t+nB_t)),losses,dt);
...
savename = [settings.name '_' settings.scenario '_N_' num2str(settings.N) '_RING_RT_' num2str(settings.simRT) '_disp_value_' num2str(disp_vals(end))];
save(savename);
