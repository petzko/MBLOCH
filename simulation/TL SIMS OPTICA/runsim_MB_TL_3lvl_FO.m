...
if(init > 0)
    ...
    dat.zUL = zUL_fit(settings.bias/10)/2;
...
while(dat.t< tEnd)
   ...
   %%%%% end of setting up the TL PARAMS %%%%%
    ...
    if(mod(iter_ctr,1000) == 0)
        ...
        subplot(3,1,1)
        plot(dat.x,[abs(dat.U).^2+ abs(dat.V).^2]);
        subplot(3,1,2)
        ax = plotyy(dat.x,[dat.v_TL*10],dat.x,dat.i_TL);

%         ax = plot(dat.x,[dat.v_TL]);

%         set(ax(1),'Ylim',[7 14]);
        title(info.SIMTYPE);
        subplot(3,1,3);
        %plots the populations and the current density 
        plot(dat.x,[dat.r110,dat.r330,dat.r220,dat.rRES],dat.x,dat.J_TL);
%         plot(dat.x,dat.J_TL);

        getframe;
        display(['V_s: ' num2str(dat.u_t(dat.t))]);
        display(['v_0: ' num2str(dat.v_TL(1))]);
        display(['i-in: ' num2str(dat.i_TL(1))]); 
        display(['i-out: ' num2str(trapz(dat.x,dat.J_TL))]);

    end
    %%%% obtain the field, field intensity and the total population at position "idx" ...
    
    if(mod(iter_ctr,iterperrecord) == 0)
