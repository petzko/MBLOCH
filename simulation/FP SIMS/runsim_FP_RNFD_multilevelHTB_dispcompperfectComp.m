...
if(settings.deph>0)
    gamma_13 = 1/settings.Tdeph_1; %% dephsing of the resonant tunneling transition
...
info.Ltot = Ltot;
info.N = N;
info.SIMTYPE = ['WITH PERFECT PHASE COMP'] ;
info.l_0 = l_0;
...
while(t< tEnd)
    ...
    if(mod(iter_ctr+1,checkptIter) == 0 )
        checkptname = ['CHCKPT_' settings.name '_' settings.scenario '_WITH_PERFECT_PHASE_COMP_N_' num2str(settings.N) '_FP'];
        save(checkptname);
    end
    ...
    Ynew = (win.*abs(Ynew).*exp(1i*slope*k_+1i*unwrap(angle(Yold))));
...
savename = [settings.name '_' settings.scenario '_WITH_perfect_compensation_N_' num2str(settings.N) '_FP_RT_' num2str(settings.simRT)];
save(savename);
