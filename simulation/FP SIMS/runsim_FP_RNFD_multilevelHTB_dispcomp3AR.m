function [ savename ] = runsim_FP_RNFD_multilevelHTB_dispcomp( scenarioFile,simDataFile,varargin )
...
if(init > 0)
    ...
    l_0 = settings.loss*100/(1/lch)*0.3;
    
%     disp_vals = [ 24.3995   24.3995    8.0000    4.0907    2.0000   -1.6904 1]
    %   disp_vals = [ 19.0695   29.7295    1.2550    1.2550    0.7316    0.7316 2]
    %             disp_vals = [22.3192   26.4798    2.0056    2.0056   -0.5044 -0.5044 3];
    disp_vals = [22.3192   26.4798    2.0056    2.0056   -0.4044 -0.4044 6];
    
    %             disp_vals = [24.3995   24.3996    3.7712    7.9998   -1.4297    2.0000 4];
    %             disp_vals = [24.3995   24.3994    5.6315    7.9966   -3.6099    4.9922 5]
    %a-system
    wA = disp_vals(1); gammaA = disp_vals(3); rA =disp_vals(5); dnA_eq = rA*dN32_eq; TA = 0.5;
    dA = +1i*(E0 - wA)- gammaA; %
    
    dnA = dnA_eq*ones(N,1);
    nA_p = zeros(N,1); nA_m = zeros(N,1);
    dnA_solver = MS(nr_steps,N,[],dnA);
    nA_p_solver = MS(nr_steps,N,[],nA_p);
    nA_m_solver = MS(nr_steps,N,[],nA_m);
    
    
    
    %b-system
    wB = disp_vals(2); gammaB = disp_vals(4); rB =  disp_vals(6); dnB_eq = rB*dN32_eq; TB = 0.5;
    dB = +1i*(E0 - wB)- gammaB; %
    
    dnB = dnB_eq*ones(N,1);
    nB_p = zeros(N,1); nB_m = zeros(N,1);
    dnB_solver = MS(nr_steps,N,[],dnB);
    nB_p_solver = MS(nr_steps,N,[],nB_p);
    nB_m_solver = MS(nr_steps,N,[],nB_m);
...
info.Ltot = Ltot;
info.N = N;
info.SIMTYPE = 'WITH 2lvl- DISP. COMP.';
info.l_0 = l_0;
...
%         getframe;
    ...
    %inversions a-system
    interA = conj(U).*nA_p+conj(V).*nA_m;
    dnA_t = 1i*(interA-conj(interA))-(dnA-dnA_eq)/TA;
    dnA_solver.make_step(dnA_t,dt);
    
    %inversions b-system
    interB = conj(U).*nB_p+conj(V).*nB_m;
    dnB_t = 1i*(interB-conj(interB))-(dnB-dnB_eq)/TB;
    dnB_solver.make_step(dnB_t,dt);
    
    % coherences a-system
    nA_p_t = dA*nA_p+1i/2*U.*dnA;
    nA_p_solver.make_step(nA_p_t,dt);
    
    nA_m_t = dA*nA_m+1i/2*V.*dnA;
    nA_m_solver.make_step(nA_m_t,dt);
    
    % coherences b-system
    nB_p_t = dB*nB_p+1i/2*U.*dnB;
    nB_p_solver.make_step(nB_p_t,dt);
    
    nB_m_t = dB*nB_m+1i/2*V.*dnB;
    nB_m_solver.make_step(nB_m_t,dt);
    ...
    U = U_solver.make_step(-1i*c*(n32p+nA_p+nB_p),-1i*c*(n32p_t+nA_p_t+nB_p_t),losses,dt);
    V = V_solver.make_step(-1i*c*(n32m+nA_m+nB_m),-1i*c*(n32m_t+nA_m_t+nB_m_t),losses,dt);
    ...
    % system a and b inversions and coherences!
    dnA = dnA_solver.get_latest_solution();
    dnB = dnB_solver.get_latest_solution();
    
    nA_p = nA_p_solver.get_latest_solution();
    nA_m = nA_m_solver.get_latest_solution();
    
    nB_p = nB_p_solver.get_latest_solution();
    nB_m = nB_m_solver.get_latest_solution();
...
savename = [settings.name '_' settings.scenario '_N_' num2str(settings.N) '_FP_' num2str(settings.simRT) '_disp_value_' num2str(disp_vals(end)) ];
save(savename);
end
