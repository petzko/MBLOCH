function [ savename ] = runsim_FP_RNFD_multilevelHTB_dispcompPHASE( scenarioFile,simDataFile,varargin )
...
info.Ltot = Ltot;
info.N = N;
info.SIMTYPE = ['WITH PHASE COMP. after ' num2str(recordPhaseIterations) ' iter'] ;
info.l_0 = l_0;
...
while(t< tEnd)
    ...
    if(mod(iter_ctr+1,checkptIter) == 0 )
        checkptname = ['CHCKPT_' settings.name '_' settings.scenario '_WITH_CONST_WINPHASE_LATEST_COMP_N_' num2str(settings.N) '_FP'];
        save(checkptname);
    end
    
    if(iter_ctr == startphasemeas)
        U_lin_prev = [U(1:N-1);V(N:-1:2)];
    end
    
    if (iter_ctr-startphasemeas == recordPhaseIterations)
        
        Y1 = transform(real(U_lin_prev)); Y2 = transform(real([U(1:N-1);V(N:-1:2)]));
...
%         Ymod = fftshift(win.*exp(-1i*[-flipud(Psi_const);Psi_const]/nconst));  % Specify the filter response at the freqs in f1.
    end
...
savename = [settings.name '_' settings.scenario '_WIITH_CPC_WIDEWIN_SMALL-LW_after_' num2str(recordPhaseIterations) '_N_' num2str(settings.N) '_FP_RT_' num2str(settings.simRT)];
save(savename);
