...
dat.rates1 =   r11.*dat.W(:,dat.INJ,dat.DEPOP) +  r33.*dat.W(:,dat.ULL,dat.DEPOP)+...
    r22.*dat.W(:,dat.LLL,dat.DEPOP);
for p=1:dat.N_rest
    shb_pp = dat.rRES_SHB{p}.*exp(2*1i*dat.x*dat.E0/dat.c);
    dat.rates1 = dat.rates1 + (dat.rRES{p}+2*real(shb_pp)).*dat.W(:,dat.global_idx_rest(p),dat.DEPOP);
end
...
% %     %voltage wave 
    ...
    dat.i_TL(1) = (dat.Vs(dat.t)-dat.v_TL(1)*dat.Lp_mm*dat.Np)/(dat.Z_kV_A+dat.Z0)/dat.width_mm ; 
    %right side -> OC
    dat.i_TL(end) = 0;
% %     
