...
simnames = {['FWM-central-lobes-test-3waveslong']};
scen_ctr = 0;
...
% % %             central lobes
                ...
                n2 = round(k2/dKappa);
                k2 = n2*dKappa;
                U = exp(1i*k1*x) +exp(1i*k2*x);
            
% %           one lobe 3wave
%             w_c = 2*pi*(4.202-E0/2/pi); 
%             k2 = (abs(w_c))/c;
%             k2 = round(k2/dKappa)*dKappa;
%             k1 = k2-dKappa;
%             k3 = k2+dKappa;
%             U = exp(1i*k1*x) +exp(1i*k2*x)+exp(1i*k3*x);
%             
            U = ampl * U / max(abs(U)); 
        ...
        while(t< tEnd)
            ...
            if(mod(iter_ctr,100) == 0)
                ...
                plotyy(x,[real(U),real(U_orig)],x,[r11,r33,r22,populations]);
                legend(legendinfo);
            ...
            %nondispersive wave!
            U_orig = uncompensated_wave_solver.make_step(-1i*c*n32*0,-1i*c*n32_t*0,losses*0,dt);
            U_orig = uncompensated_wave_solver.set_bdry(U_orig(end),'no');
        ...
        savename = [settings.name '__FWM' simnames{tau_idx} '_' strrep(num2str(ampl),'.','p') '_RING_' num2str(N) '_' strrep(num2str(Tdeph_1),'.','p') '_'  strrep(num2str(Tdeph_2),'.','p') '_'  strrep(num2str(Tdeph_3),'.','p')];
