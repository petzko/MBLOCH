...
%             end
        ...
        while(t< tEnd)
            ...
            if(strcmp(compensation,'constant'))
                if (iter_ctr+1==recordPhaseIterations)
                    ...
                    Ymod = fftshift(exp(-1i*[-flipud(Psi_const);Psi_const]/nconst));  % Specify the filter response at the freqs in fconst','Ymod','k_');
                end
            end
            ...
            if(mod(iter_ctr, 10) == 0)
                ...
                subplot(2,1,1);
                plot(abs([U(1:N-1);V(N:-1:2)]))
                subplot(2,1,2);
                plot(x,[abs(real(U)),abs(imag(U))]);
