...
if(init > 0)
    ...
    f_p = aE_in(x,0); f_p = 0*f_p/max(abs(f_p));  f_m = 0*f_p;
    ...
    idx = round(N/2); % the index of the predefined point we refered to !
    ...
    v_0 = bias;
    v_TL_new = v_0*ones(N,1); % transmission line voltage per unit length (in units kV/mm);
    v_TL_old = v_TL_new;
...
while(t< tEnd)
    ...
    if(mod(iter_ctr,plotCtr) == 0)
        ...
        subplot(2,1,2) ;
        plotyy(x,v_TL_new*10,x,J_TL*100); legend('Bias (kv/cm)','curr density(A/cm^2)');
        getframe;
    ...
    r110_t = 1i*O13.*(conj(r310)-r310) +(W_fit{ULL,INJ}(v_TL_new) + W_fit{ULL,DEPOP}(v_TL_new)).*r330 + ( W_fit{LLL,INJ}(v_TL_new)+W_fit{LLL,DEPOP}(v_TL_new) ).*r220 - G(:,INJ).*r110;
    for p = 1:N_rest
        p_glob_idx = idx_rest(p);
    ...
    r330_t = 1i*O13.*(r310 - conj(r310)) +1i/2.*(lmInteraction-conj(lmInteraction)) + r110.*W_fit{INJ,ULL}(v_TL_new) + r220.*W_fit{LLL,ULL}(v_TL_new) - G(:,ULL).*r330;
    for p = 1:N_rest
        p_glob_idx = idx_rest(p);
    ...
    if(shb > 0 )
        r11p_t = 1i*O13.*(conj(r31m)-r31p) + (W_fit{ULL,INJ}(v_TL_new)+W_fit{ULL,DEPOP}(v_TL_new)).*r33p+ (W_fit{LLL,INJ}(v_TL_new)+W_fit{LLL,DEPOP}(v_TL_new)).*r22p - (G(:,INJ)+diffusion).*r11p;
        r11p_solver.make_step(r11p_t,dt);
        
        r33p_t = 1i*O13.*(r31p-conj(r31m))+1i/2*(f_m.*conj(n23p) -conj(f_p).*n23m) +  W_fit{INJ,ULL}(v_TL_new).*r11p + W_fit{LLL,ULL}(v_TL_new).*r22p - (G(:,ULL)+diffusion).*r33p;
        r33p_solver.make_step(r33p_t,dt);
        
        r22p_t = -1i/2*(f_m.*conj(n23p) -conj(f_p).*n23m) +  W_fit{INJ,LLL}(v_TL_new).*r11p + W_fit{ULL,LLL}(v_TL_new).*r33p - (G(:,LLL)+diffusion).*r22p;
