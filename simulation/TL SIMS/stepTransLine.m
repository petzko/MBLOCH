...
%     v_tmp(2:end-1) = 2*dat.v_TL(2:end-1)-dat.v_TL_old(2:end-1) + dat.Acoeff*(dat.v_TL(3:end)-2*dat.v_TL(2:end-1)+dat.v_TL(1:end-2)) - dat.Bcoeff*dat.J_TL_t(2:end-1); 
%     v_tmp(1) = dat.v0_t(dat.t); 
%     %open circuit bdry cond. (reflection)
%     %explicit:  v_tmp(end) = 2*dat.v_TL(end)-dat.v_TL_old(end)+2*dat.Acoeff*(dat.v_TL(end-1)-dat.v_TL(end))- dat.Bcoeff*dat.J_TL_t(end); 
%     %implicit:  v_tmp(end) = 1/(1+2*Acoeff)*(2*v_TL_new(end)-v_TL_old(end)+2*Acoeff*v_tmp(end-1)); 
%     %absorbing bdry cond:
%     v_tmp(end)= dat.v_TL(end-1)+(settings.nTHz-settings.nRF)/(settings.nTHz+settings.nRF)*(v_tmp(end-1)-dat.v_TL(end));
...
%     
    ...
    v_tmp(1:end-1) = dat.v_TL(1:end-1)-dat.Ecoeff*(dat.i_TL(2:end)-dat.i_TL(1:end-1))-dat.Fcoeff*dat.J_TL(1:end-1);
    
    
    %set bdry conditions for v at right end! 
      %open circuit bdry cond. (reflection)
%     %explicit:  
    v_tmp(end) = 2*dat.v_TL(end)-dat.v_TL_old(end)+2*(settings.nTHz/settings.nRF)^2*(dat.v_TL(end-1)-dat.v_TL(end)); 
%     v_tmp(end) = v_tmp(end-1);
