classdef LaxSolverII < handle
    ...
    methods
        function obj = LaxSolverII(N,dx,direction,velocity, U_0)
        ...
        function res = make_step(obj,f,f_t,k,dt)
            ...
            if(obj.PROPERTIES(4) > 0 ) 
                ...
                % alias for the solution vector with zeroth order interpolation 
                % at the right boundary!
                E = [obj.SOLUTION; obj.SOLUTION(end)];
                
                %%% new
                f_x = (f(2:N) - f(1:N-1))/(dx);
                Ux = (E(3:N+1) - E(1:N-1))/(2*dx); % gpu array already...
                Uxx = (E(3:N+1) -2* E(2:N) + E(1:N-1))/(dx*dx);
                obj.SOLUTION(2:N) = E(2:N) + dt*(-c*Ux+f(2:N)+k(2:N).*E(2:N))+...
                (dt^2/2)*((c^2)*Uxx+ (f_t(2:N)-c*f_x) +k(2:N).*(-2*c*Ux +f(2:N)+k(2:N).*E(2:N)));
                
            else
                % alias for the solution vector with zeroth order interpolation 
                % at the left boundary!
                E= [obj.SOLUTION(1); obj.SOLUTION]; % alias for the solution vector    
               
                f_x = (f(2:N) - f(1:N-1))/(dx);
                Vx = (E(3:N+1) - E(1:N-1))/(2*dx);
                Vxx = (E(3:N+1) -2* E(2:N) + E(1:N-1))/(dx*dx);
    
                obj.SOLUTION(1:N-1) = E(2:N) + dt*(c*Vx+f(1:N-1)+k(1:N-1).*E(2:N))+...
                    (dt^2/2)*((c^2)*Vxx + f_t(1:N-1)+c*f_x +k(1:N-1).*(2*c*Vx  + f(1:N-1) + k(1:N-1).*E(2:N)));
            end
